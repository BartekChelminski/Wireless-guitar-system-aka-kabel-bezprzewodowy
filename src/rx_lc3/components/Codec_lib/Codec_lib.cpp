...
//****************************************************************************
// UWAGA: Biblioteka została zmodyfikowana przez autorów projektu tak, żeby
// działała na esp-idf, a nie na ARDUINO, jak pierwotnie została napisana.
//****************************************************************************
...
bool WM8960::begin(i2c_port_t wirePort)
{
    _i2cPort = wirePort;
...
// Sprawdza czy urzadzenie zglasza sie na magistrali I2C
bool WM8960::isConnected()
{
    // Klasyczny "Ping" I2C w czystym ESP-IDF.
    // Wysyłamy sygnał START, adres urządzenia z bitem zapisu (0), sygnał STOP.
    // Jeśli urządzenie fizycznie jest na magistrali, odeśle sygnał ACK.
    
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (_deviceAddress << 1) | 0, true);
    i2c_master_stop(cmd);
...
// Glowna funkcja wysylajaca dane do kodeka
bool WM8960::writeRegister(uint8_t reg, uint16_t value)
{
    ...
    esp_err_t err = i2c_master_write_to_device(_i2cPort, _deviceAddress, data, 2, pdMS_TO_TICKS(100));
    return (err == ESP_OK);
}
