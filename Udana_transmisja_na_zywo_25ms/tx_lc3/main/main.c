...
// Piny I2S (jak w projekcie Arduino MODDED_ADC_config)
#define I2S_BCLK_PIN    4   // D3
#define I2S_LRCK_PIN    3   // D2
#define I2S_DIN_PIN     2   // D1 (TX_SDA kodeka → DIN ESP32)

// Piny I2C (jak w projekcie Arduino)
...
// Więcej ramek niż przy 7.5 ms, ale ten sam rozmiar bufora w bajtach
...
// Bufor stereo I2S: 360 ramek × 2 kanały = 720 próbek (statyczny — nie na stosie)
...
void setup_i2s(void) {
    // Kanał RX (odbiór danych z kodeka ADC)
    ...
    // Konfiguracja I2S jak w projekcie Arduino:
    // 16-bit, stereo (kodek wysyła L+R, dane na prawym kanale), Philips standard
...
// --- 1. PRZECHWYTYWANIE AUDIO Z I2S (Rdzeń 1) ---
...
// --- 2. KODER LC3 (Rdzeń 0) ---
// Kodowanie jest CPU-intensywne — osobny rdzeń od wysyłki ESP-NOW
void audio_encode_task(void *pvParameters) {
    ...
    audio_packet_t tx_packet;
            ...
            // Przekazanie gotowego pakietu do taska wysyłki
            xQueueSend(encoded_queue, &tx_packet, 0);
        }
    }
}

// --- 3. WYSYŁKA ESP-NOW (Rdzeń 1) ---
// Lekki task — tylko wysyła gotowe pakiety
...
void app_main(void) {
    ...
    // Ring buffer PCM: odsprzęgniecie I2S od kodowania LC3
    audio_ringbuf = xRingbufferCreate(RINGBUF_TOTAL_BYTES, RINGBUF_TYPE_BYTEBUF);

    // Kolejka zakodowanych pakietów: odsprzęgniecie kodowania od wysyłki ESP-NOW
    encoded_queue = xQueueCreate(8, sizeof(audio_packet_t));

    // Pipeline na dwóch rdzeniach:
    // Rdzeń 1: I2S capture + LC3 encode (capture czeka na DMA, encode używa CPU między odczytami)
    // Rdzeń 0: ESP-NOW send (razem ze stosem WiFi który jest na rdzeniu 0)
